...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    auto borderColor = ImColor(255, 255, 255, 255);       // Bright white for the border
    auto topColor = ImColor(20, 20, 20, 255);            // Dark gray for the top gradient
    auto bottomColor = ImColor(40, 40, 40, 255);         // Slightly lighter gray for the bottom gradient
    auto topColorHovered = ImColor(40, 40, 40, 255);     // Slightly lighter gray for hovered top gradient
    auto bottomColorHovered = ImColor(60, 60, 60, 255);  // Even lighter gray for hovered bottom gradient
    auto checkedTopColor = ImColor(255, 255, 255, 255);  // Bright white for the checked top gradient
    auto checkedBottomColor = ImColor(150, 150, 150, 255); // Light gray for the checked bottom gradient
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Slider size (thin progress line and small handle)
    const float sliderHeight = 8.0f;  // Thin line
    const float handleRadius = 6.0f;  // Smaller handle circle
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(CalcItemWidth(), sliderHeight));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0 ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    // Calculate the normalized slider value
    float t = (*v - v_min) / (v_max - v_min);  // Current value as a percentage

    // Animation interpolation (smoothing)
    float anim_speed = 10.0f;
    static float prev_value = 0.0f;
    float anim_t = ImSaturate(g.Time * anim_speed);
    t = ImLerp(prev_value, t, anim_t);
    prev_value = t;

    // Update value on press/drag
    if (pressed || held)
    {
        const float mouse_pos = GetMousePos().x - frame_bb.Min.x;
        t = ImClamp(mouse_pos / frame_bb.GetWidth(), 0.0f, 1.0f);
        *v = ImLerp(v_min, v_max, t);
    }

    // Colors (dark purple with glow)
    ImColor trackColor = ImColor(60, 10, 10, 255);             // Dark red for the thin progress line
    ImColor fillColor = ImColor(140, 30, 30, 255);             // Brighter red for filled part
    ImColor handleColor = ImColor(200, 60, 60, 255);           // Bright red for the small handle
    ImColor glowColor = ImColor(255, 100, 100, 120);           // Red glow effect for handle


    // Draw the slider track (thin line)
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, trackColor, sliderHeight * 0.5f);

    // Draw the filled section (thin line with progress)
    ImRect fill_bb(frame_bb.Min, ImVec2(frame_bb.Min.x + t * frame_bb.GetWidth(), frame_bb.Max.y));
    window->DrawList->AddRectFilled(fill_bb.Min, fill_bb.Max, fillColor, sliderHeight * 0.5f);

    // Draw the handle (small circular knob with glow)
    ImVec2 handle_pos = ImVec2(fill_bb.Max.x, frame_bb.Min.y + sliderHeight / 2.0f);
    window->DrawList->AddCircleFilled(handle_pos, handleRadius, handleColor);

    // Draw the glowing effect around the handle
    window->DrawList->AddCircleFilled(handle_pos, handleRadius + 3.0f, glowColor);

    // Render the label next to the slider
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y), label);

    // Optionally display the formatted value text in the center
    RenderTextClipped(frame_bb.Min, frame_bb.Max, format, NULL, NULL, ImVec2(0.5f, 0.5f));
