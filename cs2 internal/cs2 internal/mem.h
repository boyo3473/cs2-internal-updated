...
namespace mem {
	...
	type Read(uintptr_t pointerStatic) {
		type value = { };
		ReadProcessMemory(mem::processHandle, (LPVOID)pointerStatic, &value, sizeof(type), NULL);
		return value;
	}
	...
	bool Write(uintptr_t pointerStatic, type value) {
		...
		return WriteProcessMemory(mem::processHandle, (LPVOID)pointerStatic, &value, sizeof(type), NULL);
